...
class Pipeline(object):
    ...
    def __init__(self, site_id, site_name, *field_id):
    ...
    def process_field(self, item):
        try:
            ...
            elif type(item['field']) is str or type(item['field']) is int:
                field_obj = db.db_session.query(db.Field).filter(db.Field.id == item['field']).first()
                field_list.append(field_obj)

            elif type(item['field']) is list:
                for x in item['field']:
                    field_obj = db.db_session.query(db.Field).filter(db.Field.id == x).first()
                    field_list.append(field_obj)
            else:
                pass
    ...
    def upload_item(self, item, structure):
        try:
            ...
            # field = db.db_session.query(db.Field).filter(db.Field.id == self.field_id).first()
            start_time = datetime.datetime.today() - datetime.timedelta(days=20)
            news_set = self.orm_sort(db.db_session.query(db.News.link).filter(db.News.site_id == self.site_id).all())
            # news_set = self.orm_sort(db.db_session.query(db.News.link).filter(and_(db.News.site_id == self.site_id, db.News.datetime >= start_time)).all())

            if item['title'] is not None:  #判断如果item['title']不为空则执行插入操作
                if item['link'] not in news_set and item['link'] not in self.item_set:  #如果该link已经存在于数据表内略过
                    new_news = db.News(
                        link=item['link'],
                        title=item['title'],
                        hot=item['hot'],
